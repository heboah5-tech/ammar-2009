...
export default function DentalWorkPage() {
  const [generalCost, setGeneralCost] = useState<number>(0)
  const [generalCostInput, setGeneralCostInput] = useState<string>("0")
  const [works, setWorks] = useState<DentalWork[]>([])
  const [loading, setLoading] = useState(true)
  const [savingBudget, setSavingBudget] = useState(false)

  const [form, setForm] = useState<DentalWork>({
    doctorName: "",
    workType: "",
    teethCount: 1,
    color: "",
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const budgetDoc = await getDoc(doc(db, "settings", "materialsBudget"))
      if (budgetDoc.exists()) {
        const v = budgetDoc.data().value || 0
        setGeneralCost(v)
        setGeneralCostInput(String(v))
      }
  ...
  const totalSpent = works.reduce((sum, w) => sum + (w.materialCost || 0), 0)
  const remaining = generalCost - totalSpent

  const handleSaveBudget = async () => {
    setSavingBudget(true)
    try {
      const v = parseFloat(generalCostInput) || 0
      await setDoc(doc(db, "settings", "materialsBudget"), {
        value: v,
        updatedAt: Timestamp.now(),
      })
      setGeneralCost(v)
      alert("تم حفظ التكلفة العامة")
    } catch (e) {
      console.error(e)
      alert("خطأ في حفظ التكلفة")
    } finally {
  ...
  return (
    <div className="container mx-auto py-6 sm:py-8 px-4 max-w-6xl" dir="rtl">
      ...
      <Card className="border border-slate-200 shadow-md rounded-lg overflow-hidden mb-6">
        <CardHeader className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white p-3 sm:p-6">
          <CardTitle className="text-base sm:text-lg">التكلفة العامة للمواد</CardTitle>
        </CardHeader>
        <CardContent className="pt-4 sm:pt-6 p-3 sm:p-6">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 sm:gap-4 items-end">
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                min="0"
                step="0.01"
                value={generalCostInput}
                onChange={(e) => setGeneralCostInput(e.target.value)}
            ...
            <Button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 text-white flex items-center justify-center gap-2 rounded-lg"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-4 border-t-2 border-slate-200">
            <div className="bg-blue-50 border-2 border-blue-200 p-3 rounded-lg text-center">
              <p className="text-xs sm:text-sm text-slate-700 mb-1">التكلفة العامة</p>
              <p className="text-lg sm:text-xl font-bold text-blue-700">{generalCost.toFixed(2)}</p>
            </div>
            <div className="bg-orange-50 border-2 border-orange-200 p-3 rounded-lg text-center">
              <p className="text-xs sm:text-sm text-slate-700 mb-1">إجمالي المصروف</p>
              <p className="text-lg sm:text-xl font-bold text-orange-700">{totalSpent.toFixed(2)}</p>
            </div>
            <div
              className={`${remaining < 0 ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"} border-2 p-3 rounded-lg text-center`}
            >
              <p className="text-xs sm:text-sm text-slate-700 mb-1">المتبقي</p>
              <p className={`text-lg sm:text-xl font-bold ${remaining < 0 ? "text-red-700" : "text-green-700"}`}>
                {remaining.toFixed(2)}
              </p>
            </div>
          </div>
              ...
              <table className="w-full text-xs sm:text-sm">
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    <th className="p-2 sm:p-3 text-center font-bold text-slate-700">التكلفة</th>
                ...
                <tfoot className="bg-slate-100 font-bold">
                  <tr>
                    ...
                    <td className="p-2 sm:p-3 text-center text-orange-700">{totalSpent.toFixed(2)}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
